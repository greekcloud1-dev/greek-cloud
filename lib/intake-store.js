...
function client() {
  return new S3Client({
    ...
    forcePathStyle: true,
    // The SDK's newer default adds a flexible-checksum header (e.g.
    // x-amz-checksum-crc32) to every request and signs it in. Supabase's S3
    // implementation does not recompute the signature the same way with that
    // header present, so every upload failed with SignatureDoesNotMatch until
    // this was turned off.
...
export async function putIntakeFile(submissionId, name, file, contentType) {
  ...
  // A Blob/File's own stream can only be read once and cannot be rewound, but
  // the S3 SDK needs to re-read the body to compute a checksum -- so it must
  // be buffered first rather than handed to PutObjectCommand as a stream.
  const body = typeof file.arrayBuffer === 'function' ? new Uint8Array(await file.arrayBuffer()) : file;
  try {
    await client().send(
